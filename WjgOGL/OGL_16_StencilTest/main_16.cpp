...
int main()
{
	...
	//符合GL_NOTEQUAL的通过模板测试，参考值为1(ref)
	...
	//地面
	float planeVertices[] = {	
		//地面纹理坐标大于1，图像在表面上重复出现，GL_REPEAT
	...
	原理概述：
		...
		现在利用第一个图形那部分模板值全是1的特点，运用函数设置为当模板值不等于1时，第二个图形进行绘制。那么效果就是，
		第一个图形正常绘制，第二个图形仅有比第一个图形大的那部分绘制出来，所以看上去就是多了一个边缘。
	*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//描述外轮廓的的顶点着色器,可以继续使用之前的观察者矩阵和投影矩阵，模型矩阵的缩放系数变大就行了
		OutlineShader.use();
		...
		//开始绘制两个正常的箱子，开启模板缓冲的写入，GL_ALWAYS意思为总是通过模板测试
		...
		//绘制两个放大的箱子，关闭模板缓冲的写入
		//GL_NOTEQUAL意思为不等于才通过模板测试,我们在绘制两个正常的箱子的时候已经把他们的片段的模板缓冲设为了1
		//关闭深度测试，是为了不影响轮廓显示
		glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
		glStencilMask(0x00);
		glDisable(GL_DEPTH_TEST);
		OutlineShader.use();
		//两个放大的箱子，只有外轮廓通过了模板测试
		float scale = 1.05;
		glBindVertexArray(cubeVAO);
		//不需要用到纹理
		//glActiveTexture(GL_TEXTURE0);
