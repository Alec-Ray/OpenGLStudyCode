...
int main()
{
	...
	//PBR PbrShader
	Shader PbrShader("Shader/36/PbrLighting.vs", "Shader/36/PbrLighting.fs");
	//将等柱状投影图转换为立方体贴图
	Shader EquirectangularToCubemapShader("Shader/36/CubeMap.vs", "Shader/36/Equirectangular_to_cubemap.fs");
	//通过生成的Cubemap来渲染天空盒
	Shader SkyboxShader("Shader/36/Skybox.vs", "Shader/36/Skybox.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		{
			//每行小球的金属都一样，每一列从上到下金属度数从1.0到0.0
			PbrShader.setFloat("metallic", (float)row / (float)nrRows);
			for (int col = 0; col < nrColumns; ++col) 
			{
				//每列小球粗糙度一样，每行从左到右粗糙度从0.05开始增加，逐渐逼近1.0
		...
		//打开此段代码，可以看到我们从等柱状投影图转换得到的立方体贴图在一个立方体上
		EquirectangularToCubemapShader.use();
